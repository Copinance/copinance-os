...
Every event includes **`schema_version`** (integer, currently **1**) and **`kind`**. Parse JSON with **`copinance_os.domain.models.pipeline.agent_progress.parse_agent_progress_event`** if you need server-side validation.
...
- **`AGENT_PROGRESS_SCHEMA_VERSION`** in **`copinance_os.domain.models.pipeline.agent_progress`**: global constant for documentation; clients can log or gate UI on it.
...
| Item | Location |
|------|----------|
| Event union + `parse_agent_progress_event` | `copinance_os.domain.models.pipeline.agent_progress` |
...
**Ask AI suggestion chips** (before the user sends a question): use `generate_curated_questions_use_case` with payloads you already fetched — see [Curated questions (clients)](curated-questions-integration).
