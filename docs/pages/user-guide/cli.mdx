...
**Note:** For best results with macro indicators (rates, credit, commodities), configure `COPINANCEOS_FRED_API_KEY` in your `.env` file. The workflow will automatically use FRED for true yields/spreads and fall back to yfinance proxies if FRED is unavailable. See [Configuration](configuration#fred-api-setup) for setup instructions.
