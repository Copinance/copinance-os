...
**LLM:** Requires `get_container(llm_config=...)`. When unavailable, `questions` is empty and `meta.llm_unavailable_reason` is a typed enum (`no_llm_config`, `provider_error`, `parse_error`, `rate_limited`, `timeout`).
...
| `MarketNarrativeRequest` | `copinance_os.domain.models.narrative` | `market_index` (default `"SPY"`), `financial_literacy`, `no_cache` |
| `NarrativeResult` | `copinance_os.domain.models.narrative` | `narrative` (prose string), `literacy`, `market_index`, `generated_at`, `fallback` (`True` when LLM unavailable) |
| `GenerateCuratedQuestionsRequest` | `copinance_os.domain.models.curated_questions` | `artifact`, `payload`, `count`, `financial_literacy`, `learning_step`, `no_cache` |
| `CuratedQuestionsBlock` | `copinance_os.domain.models.curated_questions` | `questions`, `meta` (`llm_enabled`, `llm_unavailable_reason`, `cache_hit`, …) |
| `ArtifactType` | `copinance_os.domain.models.curated_questions` | Payload-shape enum for curated-question generation |
| `CuratedQuestion` | `copinance_os.domain.models.curated_questions` | Chip: `text`, `focus`, `suggested_tools`, `requires_symbol` |
| `LLMUnavailableReason` | `copinance_os.domain.models.curated_questions` | Typed fallback when `questions` is empty |
...
|------|------------|-----|
| **Tier 1 — custom `Storage`** | File-format alternatives, SQLite, S3 | Implement `Storage` ABC from `copinance_os.domain.ports.storage` and pass it as `storage_backend` to `get_container()`. `save()` must be atomic. |
