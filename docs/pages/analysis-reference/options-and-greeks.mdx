...
`OptionsChain.metadata` remains provider-facing input metadata (for example, `provider` and optional `dividend_yield`). Methodology output is now explicit and structured:
