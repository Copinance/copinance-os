...
CLI export path:
- `copinance analyze --json positioning <SYMBOL> -w near|mid` exports `RunJobResult`, where `results` is the full `OptionsPositioningResult` payload.
- Non-JSON analyze runs also persist the same structured payload under `.copinance/results/v2/`.
...
- **Library:** [Options positioning context](../getting-started/library#options-positioning-context) (I/O contracts and integration patterns)
- **Developer guide:** [Agent progress & chat integration (clients)](../developer-guide/agent-progress-client-integration) (tool grounding + methodology UX patterns)
